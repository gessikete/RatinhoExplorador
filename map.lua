...
local function setMap( )
  ...
  path.start = map:findObject("start")
end
...
local function setCharacter( )
  ...
  character.steppingX = path.start.x
  character.steppingY = path.start.y
...
local function setCamera( )
  ...
  camera:setBounds( 180, 620, 150, 620 )
...
local function finishedLoading( )
  ...
  transition.fadeOut( loading, { time = 800, onComplete = function( ) loading:removeSelf( ) loading = nil print( loading ) end } )
...
local function loadGameFile( )
  ...
  if ( gameFile ~= nil ) then 
    local stepsX = math.ceil((gameFile.character.steppingX - path.start.x)/tilesSize)
    local stepsY = math.ceil((gameFile.character.steppingY - path.start.y)/tilesSize)
    local time
