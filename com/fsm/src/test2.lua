...
local function build_transition(self, event, states, events)
  return function (...)
    local from = self.current
    local to = states[from].to or states[M.WILDCARD].to or from
    local nextEvent = states[from].nextEvent
    ...
    self.confirm = function ()
      ...
      --[[print( "---------" )
        print( "current: " .. tostring(event) )
        print( "next: " .. tostring(event.nextEvent) )
      print( "---------" )]]

     --[[ for _, e in pairs(events) do
        print( "---------" )
        print( event )
        print( "current: " .. tostring(e.name) )
        print( "next: " .. tostring(e.nextEvent) )
        print( "to: " .. tostring(e.to) )
        print( "from: " .. tostring(e.from) )
        print( "---------" )
      end]]
...
function M.create(cfg, target)
  ...
  local function add(e)
    ...
    for _, fr in ipairs(from) do
      ...
      states_for_event[event][fr] = { to = to or fr, nextEvent = nextEvent }
  ...
  for event, states in pairs(states_for_event) do
    self[event] = build_transition(self, event, states, events)
  end
